const getSignificantNum = (str: string) => {
  ...
  let leftSignificantPart = str.substring(nonZeroIdxFromRight) + decimalSuffix + eSuffix;

  // 2.Remove trailing zeros
  // numbers with e and numbers without decimals are returned w/o modifications
  if (e !== -1 || decimal === -1) return leftSignificantPart;
  ...
  let significantDigits = leftSignificantPart.substring(0, rightSignificantIdx + 1);
